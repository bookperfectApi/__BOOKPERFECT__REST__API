...
class LogForm extends Controller
{
  ...
  public function filterbyday(Request $__REQUEST)
  {
    ...
    $__ID__COUNTER = 0;
    $__DAYS__CATE = array();
    $__CURRENT__DATE = substr(Carbon::parse($__REQUEST->day)->format('d-m-Y'), 0, 10);
    foreach ($__RECORDS as $__ITEM) {
      $__DAYS__CATE[$__COUNTER] = substr($__ITEM->created_at->format('m-d-Y'), 0, 10);
      $__COUNTER = $__COUNTER + 1;
      $__ID__COUNTER = $__COUNTER;
    }

    $__DAYS__CATE = array_unique($__DAYS__CATE);
